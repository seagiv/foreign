...
func (*ExampleSvc) FullName3(t NameArgContext, res *NameRes) error {
	host, _, _ := net.SplitHostPort(jsonrpc1.HTTPRequestFromContext(t.Context()).RemoteAddr)
...
func Example() {
	...
	http.Handle("/rpc", jsonrpc1.HTTPHandler(nil))
	...
	clientTCP, err := jsonrpc1.Dial("tcp", lnTCP.Addr().String())
	if err != nil {
		panic(err)
	}
	...
	clientHTTP := jsonrpc1.NewHTTPClient("http://" + lnHTTP.Addr().String() + "/rpc")
	defer clientHTTP.Close()

	// Custom client use HTTP transport.
	clientCustomHTTP := jsonrpc1.NewCustomHTTPClient(
		"http://"+lnHTTP.Addr().String()+"/rpc",
		jsonrpc1.DoerFunc(func(req *http.Request) (*http.Response, error) {
	...
	if err == rpc.ErrShutdown || err == io.ErrUnexpectedEOF {
		...
		rpcerr := jsonrpc1.ServerError(err)

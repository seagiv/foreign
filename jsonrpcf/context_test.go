...
func (*CtxSvc) NameCtx(t NameArgContext, res *NameResCtx) error {
	*res = NameResCtx{Name: t.Fname + " " + t.Lname}
	...
	if val := jsonrpc1.HTTPRequestFromContext(t.Context()); val != nil {
...
func TestContext(t *testing.T) {
	...
	go func() {
		for {
			...
			go jsonrpc1.ServeConnContext(ctx, conn)
		}
	}()

	// Server provide a HTTP transport with context.
	http.Handle("/", jsonrpc1.HTTPHandler(nil))
	...
	clientTCPNoCtx, err := jsonrpc1.Dial("tcp", serverTCPNoCtx.Addr().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	clientTCP, err := jsonrpc1.Dial("tcp", serverTCP.Addr().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	clientHTTP := jsonrpc1.NewHTTPClient("http://" + serverHTTP.Addr().String() + "/")
	defer clientHTTP.Close()

	cases := []struct {
		client     *jsonrpc1.Client
		clientName string
		method     string
		arg        interface{}
		want       interface{}
...
func TestContextBatch(t *testing.T) {
	req := `[
		{"id":0,"method":"CtxSvc.NameCtx","params":{}},
		{"id":1,"method":"CtxSvc.NameCtx","params":{"Fname":"First","Lname":"Last"}}
		]`
	...
	rpc.ServeRequest(jsonrpc1.NewServerCodecContext(
...
func TestContextBugNoContextWithoutParams(t *testing.T) {
	cases := []struct {
		...
		{
			`{"id":0,"method":"CtxSvc.NameCtx","params":{"Fname":"First","Lname":"Last"}}`,
			map[string]interface{}{
				"Name":           "First Last",
				"TCPRemoteAddr":  "127.0.0.1",
				"HTTPRemoteAddr": "",

/*
Package jsonrpc1 implements a JSON-RPC 1.0 ClientCodec and ServerCodec
for the net/rpc package and HTTP transport for JSON-RPC 1.0.


RPC method's signature

JSON-RPC 1.0 support positional and named parameters. Which one should be
...
JSON-RPC 1.0 support result of any type, so method's result (second param)
can be a reference of any type supported by json.Marshal.

JSON-RPC 1.0 support error codes and optional extra error data in addition
...
Because of net/rpc limitations client.Call() can't return JSON-RPC 1.0
error with code, message and extra data - it'll return either one of
rpc.ErrShutdown or io.ErrUnexpectedEOF errors, or encoded JSON-RPC 1.0
error, which have to be decoded using jsonrpc1.ServerError to get error's

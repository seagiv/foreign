...
func TestHTTPServer(t *testing.T) {
	const jBad = `{}`
	const jSum = `{"id":1,"method":"Svc.Sum","params":[3,5]}`
	const jNotify = `{"method":"Svc.Sum","params":[3,5]}`
	const jRes = `{"id":1,"result":8, "error":null}`
	const jErr = `{"id":null,"error":{"code":-32600,"message":"Invalid request"}}`
	const jParse = `{"id":null,"error":{"code":-32700,"message":"Parse error"}}`
	...
	ts := httptest.NewServer(jsonrpc1.HTTPHandler(nil))
...
func TestHTTPClient(t *testing.T) {
	...
	// Don't close because of https://github.com/golang/go/issues/12262
	// defer ts.Close()
	client := jsonrpc1.NewHTTPClient(ts.URL)
	defer client.Close()
